const fs = require('fs');

const path = 'd:\\Flutter_Source\\LOVEHUB\\lib\\features\\finance\\presentation\\screens\\finance_screen.dart';
let content = fs.readFileSync(path, 'utf8');

// Fix 1: _SummaryCard - em-dash + curly quotes
const old1 = "? 'No jars yet\u2014 tap \u201c+ New Jar\u201d to start'";
const new1 = "? 'finance.noJars'.tr()";
if (content.includes(old1)) {
    content = content.replace(old1, new1);
    console.log('[OK] Replaced summary empty-state text');
} else {
    const idx = content.indexOf('jars.isEmpty');
    console.log('[DEBUG] jars.isEmpty context:', JSON.stringify(content.slice(idx, idx + 120)));
    process.exit(1);
}

// Fix 2: _EmptyJarsHint - Tap "+ New Jar" to start saving together.
const old2 = 'Tap "\u201c+ New Jar\u201d to start saving together.';
const new2 = "'finance.emptyHint'.tr()";
if (content.includes(old2)) {
    content = content.replace(old2, new2);
    console.log('[OK] Replaced empty hint text');
} else {
    const idx = content.indexOf('start saving');
    console.log('[DEBUG] start saving context:', JSON.stringify(content.slice(idx - 5, idx + 80)));
    process.exit(1);
}

// Fix 3: _JarCard - 'Add' button
const old3 = "label: 'Add'";
const new3 = "label: 'finance.addToJar'.tr()";
if (content.includes(old3)) {
    content = content.replace(old3, new3);
    console.log('[OK] Replaced Add button label');
} else {
    console.log('[DEBUG] File has no plain "label: \'Add\'" - checking:', JSON.stringify(content.slice(content.indexOf('Add') - 20, content.indexOf('Add') + 20)));
    process.exit(1);
}

fs.writeFileSync(path, content, 'utf8');
console.log('All done.');
